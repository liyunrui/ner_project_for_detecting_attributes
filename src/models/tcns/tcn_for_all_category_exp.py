...
And we apply this mdoel into our ner task: Given the item name, detect the brand names.
...
dict_category = {
    ...
    'dress':{'max_seq_length':19,'max_word_length':78},
    'women_top':{'max_seq_length':20,'max_word_length':28},

}
for category in [i for i in os.listdir('/data/ner_task/data_for_brand_detection_model/') if '.csv' not in i][1:]:
    # word embedding parameters
    filename_words_vec = "../data/wordvec/word2vec.npz"
    filename_words_voc = "../data/wordvec/{}/words_vocab.txt".format(category)
    filename_chars_voc = "../data/wordvec/{}/chars_vocab.txt".format(category)
    base_dir = "/data/ner_task/result_for_brand_detection_model/{}".format(category)
    ...
    # num residual block we used for main model architecture
    ...
    length_dict = dict_category["{}".format(category)]
    if category in ['dress','women_top']:
    ...
    nn = tcn(
        ...
        optimizer='adam',
        max_seq_len = length_dict['max_seq_length'],
        learning_rate =0.001,
        filter_widths= 3,
        ntags = 3,
        batch_size = 128,
        ...
        causal = causal,
        max_word_length = length_dict['max_word_length'],
        dim_char = 100,
